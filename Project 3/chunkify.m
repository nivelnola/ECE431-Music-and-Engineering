...
%chunks, as per the specifications. The chunk type and length are removed
%from each chunk, preserving only the data stored within.
...
trackBytes = cell(numTracks, 1);
for ticker = 1:numTracks
    trackSizes(ticker) = bytes2dec(stream(trackStart(ticker)+4:trackStart(ticker)+7));
    trackBytes{ticker} = stream(trackStart(ticker)+8:trackStart(ticker)+8+trackSizes(ticker)-1);
end
